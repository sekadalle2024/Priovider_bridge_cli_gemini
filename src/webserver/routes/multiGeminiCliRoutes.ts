...
/**
 * @swagger
 * /api/v1/cli/{profileId}/chat/completions:
 *   post:
 *     summary: Chat avec un profil spécifique (format OpenAI standard)
 *     tags: [Multi Gemini CLI]
 *     parameters:
 *       - in: path
 *         name: profileId
 ...
 *         description: Réponse du chat
 */
router.post('/:profileId/chat/completions', async (req: Request, res: Response) => {
  try {
    const profileId = req.params.profileId as string;
    const chatRequest = req.body;
 ...
 *   post:
 *     summary: Chat avec load balancing (format OpenAI standard)
 *     tags: [Multi Gemini CLI]
 *     requestBody:
 *       required: true
 *       content:
